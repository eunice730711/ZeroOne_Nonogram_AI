...
vector<vector<int> > ru[LENGTH],cu[LENGTH]; // Start position of every limit number
...
void unary_encode(int lineidx,bool column)
{
    vector<int> tmpc;
    if(column)
    {
        
        for(int i=0;i<cu[lineidx].size();i++)
        {
            tmpc.push_back(cu[lineidx][i][0]);
            clauses.push_back(tmpc);
            tmpc.clear();
            tmpc.push_back(cu[lineidx][i][1]);
            clauses.push_back(tmpc);
            tmpc.clear();
            tmpc.push_back(-cu[lineidx][i][LENGTH+1]);
            clauses.push_back(tmpc);
            tmpc.clear();

            for(int j=1;j<cu[lineidx][i].size();j++) 
            {
                tmpc.push_back(-cu[lineidx][i][j]);
                tmpc.push_back(cu[lineidx][i][j-1]);
                clauses.push_back(tmpc);
                tmpc.clear();
            }
        }
        int last = cu[lineidx].size()-1;
        int add = col[lineidx].des[last+1];
        tmpc.push_back(-cu[lineidx][last][LENGTH+1-add+1]);
        clauses.push_back(tmpc);
        tmpc.clear();
    }
    else
    {
        for(int i=0;i<ru[lineidx].size();i++)
        {
            tmpc.push_back(ru[lineidx][i][0]);
            clauses.push_back(tmpc);
            tmpc.clear();
            tmpc.push_back(ru[lineidx][i][1]);
            clauses.push_back(tmpc);
            tmpc.clear();
            tmpc.push_back(-ru[lineidx][i][LENGTH+1]);
            clauses.push_back(tmpc);
            tmpc.clear();
            for(int j=1;j<ru[lineidx][i].size();j++) 
            {
                tmpc.push_back(-ru[lineidx][i][j]);
                tmpc.push_back(ru[lineidx][i][j-1]);
                clauses.push_back(tmpc);
                tmpc.clear();
            }
        }
        int last = ru[lineidx].size()-1;
        int add = row[lineidx].des[last+1];
        tmpc.push_back(-ru[lineidx][last][LENGTH+1-add+1]);
        clauses.push_back(tmpc);
        tmpc.clear();
    }  
}
void leq_encode(int lineidx,bool column)
{
    vector<int> tmpc;
    if(column)
    {
        for(int i=0;i<cu[lineidx].size()-1;i++)
        {
            int limit_num = col[lineidx].des[i+1]+1,k=1;
            //cout << "limit_num: " << limit_num << endl;
            for(int j=1;j<cu[lineidx][i+1].size();j++)
            {
                if(limit_num>0)
                {
                    tmpc.push_back(cu[lineidx][i+1][j]);
                    clauses.push_back(tmpc);
                    tmpc.clear();
                    limit_num-=1;
                }
    ...
    {
        for(int i=0;i<ru[lineidx].size()-1;i++)
        {
            int limit_num = row[lineidx].des[i+1]+1,k=1;
            for(int j=1;j<ru[lineidx][i+1].size();j++)
            {
                if(limit_num>0)
                {
                    tmpc.push_back(ru[lineidx][i+1][j]);
                    clauses.push_back(tmpc);
                    tmpc.clear();
                    limit_num-=1;
                }
...
void black_block_encode(int lineidx, bool column)
{
    vector<int> tmpc;
    if(column)
    {
        for(int i=0;i<cu[lineidx].size();i++)
        {
            int limit_num = col[lineidx].des[i+1];
            int start = 2, add = limit_num-1, k=1;
            for(int j=1;j<=LENGTH && start <= LENGTH+1 && k<=LENGTH+1 ;j++)
            {
                tmpc.push_back(cu[lineidx][i][start]);
                tmpc.push_back(variables[j-1][lineidx]);
                if(add >0)
                {
                    add-=1;
                    clauses.push_back(tmpc); // seems nothing happened
                    tmpc.clear();
                }
                else
                {
    ...
    else
    {
        for(int i=0;i<ru[lineidx].size();i++)
        {
            int limit_num = row[lineidx].des[i+1];
            int start = 2, add = limit_num-1, k=1;
            for(int j=1;j<=LENGTH && start <= LENGTH+1 && k<=LENGTH+1 ;j++)
            {
                tmpc.push_back(ru[lineidx][i][start]);
                tmpc.push_back(variables[lineidx][j-1]);
                if(add >0)
                {
                    add-=1;
                    clauses.push_back(tmpc); // seems nothing happened
...
void empty_block_encode(int lineidx, bool column)
{
    vector<int> tmpc;
    if(column)
    {
        ...
        {
            int k=1,k1=1;
            if(i!=0) add = col[lineidx].des[i]-1;
            else add =0;
            if(i == cu[lineidx].size()-1) last = col[lineidx].des[i+1]-1;

            for(int j=1;j<=LENGTH && k<=LENGTH+1;j++)
            {
                if(i==0)
                {
                    tmpc.push_back(-cu[lineidx][i][j+1]);
                ...
                else
                {
                    ...
                    {
                        tmpc.push_back(cu[lineidx][i-1][k]);
                        tmpc.push_back(-cu[lineidx][i][j+1]);
                        tmpc.push_back(-variables[j-1][lineidx]);
                        clauses.push_back(tmpc);
                        k+=1;
                ...
                if(i == cu[lineidx].size()-1)
                {
                    ...
                    {
                        tmpc.push_back(cu[lineidx][i][k1]);
                        tmpc.push_back(-variables[j-1][lineidx]);
                        clauses.push_back(tmpc);
                        tmpc.clear();
                        k1+=1;
    ...
    else
    {
        ...
        {
            int k=1,k1=1;
            if(i!=0) add = row[lineidx].des[i]-1;
            else add =0;
            if(i == ru[lineidx].size()-1) last = row[lineidx].des[i+1]-1;
            for(int j=1;j<=LENGTH && k<=LENGTH+1;j++)
            {
                if(i==0)
                {
                ...
                else
                {
                    ...
                    {
                        tmpc.push_back(ru[lineidx][i-1][k]);
                        tmpc.push_back(-ru[lineidx][i][j+1]);
                        tmpc.push_back(-variables[lineidx][j-1]);
                        clauses.push_back(tmpc);
                        k+=1;
                ...
                if(i == ru[lineidx].size()-1)
                {
                    ...
                    {
                        tmpc.push_back(ru[lineidx][i][k1]);
                        tmpc.push_back(-variables[lineidx][j-1]);
                        clauses.push_back(tmpc);
                        tmpc.clear();
                        k1+=1;
...
void encode()
{
    newidx = LENGTH*LENGTH +1; // Starting index

    // Mapping variables of limit number 
    vector<int> rl,cl;
    ...
    //Black block encoding
    for(int i=0;i<LENGTH;i++)
    {
        black_block_encode(i,true);
        black_block_encode(i,false);
    }

    // Empty block encoding
    for(int i=0;i<LENGTH;i++)
    {
        empty_block_encode(i,true);
        empty_block_encode(i,false);
    }
